...
const ueditorPath = '/comm/ueditor1_4_3_3/';

class Ueditor extends Component {
    constructor(props){
        super(props);
        this.state = {
            ueditorLoading:false,
        };
    }
    ...
    componentDidMount(){
        this.mounted && this.setState({ ueditorLoading: true });
        this.createScript(ueditorPath + 'ueditor.config.js').then(() => {
            this.createScript(ueditorPath + 'ueditor.all.min.js').then(() => {
                ...
                this.mounted && this.setState({ ueditorLoading: false   });
                this.initEditor()
    ...
    initEditor() {
        ...
        const ueEditor = window.UE.getEditor(this.props.id, {
            /*这里是配置*/
            toolbars: [
                [
                    'anchor', //锚点
    ...
    render(){
        return (
            <Spin spinning={this.state.ueditorLoading}>
