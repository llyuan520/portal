/**
 * Created by LiLiYuan on 2017/9/27.
 */
...
export default {
    ...
    password:{
        pattern:/(?![0-9]+$)(?![a-zA-Z]+$)(?!(![0-9A-Za-z])+$)\S{6,32}/, // /^[\@A-Za-z0-9\!\#\$\%\^\&\*\.\~]{6,20}$/
    ...
    email:{
        pattern:/^([_a-z0-9]|[.]|[-])+@(([_a-z0-9]|[-])+\.)+[a-z0-9]+$/i, // /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/
        message:'非法电子邮件地址'
    },
    postcode:{
        pattern:/^(\d{6})$/,
    ...
    number:{
        pattern:/^[-]{0,1}[1-9]+[0-9]{0,}$|^[-]{0,1}[1-9]+[0-9]{0,}[.][0-9]+$|^[-]{0,1}[0]{0,1}[.][0-9]+$|^[0]{1,1}$/,  // /^[0-9]*$/
        message:'必须输入数字'
    },
    nonNegative_number:{
